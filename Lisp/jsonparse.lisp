...
(defun jsonaccess (json &rest fields)
  (let ((key (first fields))
    ...
    (cond ((null key) json)
          ((null toptype) (error "ERROR: Element cannot be accessed"))
          ((and (eql toptype 'JSONOBJ) (typep key 'string)) 
           (let ((nexthop (jsonobj-get json key)))
             (if (null nexthop)
                 (error (format NIL "ERROR: Cannot access to attribute < ~d >" key))
                 (apply 'jsonaccess (cons nexthop (rest fields)))) ))
          ( (and (eql toptype 'JSONARRAY) (typep key 'number))
           (let ((nexthop (nth key (rest json)))) 
             (if (null nexthop)
                 (error (format NIL "ERROR: Index ~d out of bounds." key))
                 (apply 'jsonaccess (cons nexthop (rest fields))) )))
          ( T (error (format NIL "ERROR: < ~d > is not a valid identifier" key))) )))
; JSONREAD
...
(defun string-dumping-json (json)
  (let ((toptype (if (typep json 'cons) (first json)))
    ...
    (cond ((null toptype) (error "ERROR: Impossible dumping JSON"))
          ((eql toptype 'JSONOBJ) 
           (concatenate 'string 
                        "{" 
